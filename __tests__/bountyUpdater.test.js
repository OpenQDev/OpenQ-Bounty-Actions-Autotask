...
describe('bountyUpdater', () => {
	it('BountyCreated', async () => {
		...
		// ACT
		const result = await bountyUpdater('BountyCreated', 'https://staging.openq.dev/api', 'apiSecret123', { bountyAddress: '0xaddress', bountyId: 'bountyId123', organization: 'orgId123', bountyType: 0 }, pat, invoiceUrl, coinApiUrl, mockGetIssue, mockCreateNewBounty, mockUpdateBountyValuation, mockGetValueFromBalance);
	...
	it('TokenBalanceClaimed', async () => {
		...
		expect(value).toEqual({
			...
			'variables': {
				'address': 'oxbounty',
				'tvc': { 'coinAPIReq': { 'tokenVolumes': { '0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270': { 'decimals': 18, 'volume': '1000' } } }, 'coinApiUrl': 'coinApiUrl123' },
				'tvl': 0
			}
		}
		);
