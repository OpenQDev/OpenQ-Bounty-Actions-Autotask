...
const getSubgraphBounty = require("./getSubgraphBounty");
const updateRequestStateToClosed = require("./updateRequestToClosed");
const getUser = require("./getUser");
const decodeSupportingDocsData = require("./decodeSupportingDocsData");
const { ethers } = require("ethers");
const getBountyTypeFromHexOrString = require("./utils/getBountyTypeFromHexOrString.js");
    ...
    try {
      ...
      switch (eventType) {
        ...
        case "SupportingDocumentsCompleteSet": {
          const [tier, bool] = decodeSupportingDocsData(
            params.data,
            params.bountyType
          );

          const { data: bountyData } = await getSubgraphBounty(
            params.bountyAddress.toLowerCase()
          );
          const tierInt = parseInt(getBountyTypeFromHexOrString(tier));
          const winner = bountyData.bounty.tierWinners?.[tierInt];
          // rn I know what tier and what user the request is for.
          const apiUser = await getUser(baseUrl, openqApiSecret, winner);
          const requestingUserId = apiUser.data.user.id;
